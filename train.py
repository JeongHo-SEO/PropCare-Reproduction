...
from evaluator import Evaluator
    # from sklearn.metrics import mean_absolute_error # not used
...
def prepare_data(flag):
    ...
    if dataset == "d":
        ...
        # modify the m1-100k path << 이부분 수정해주세요 로컬환경에 맞게. [PROPCARE 폴더를 최상단으로 열면 수정 필요 없습니다.]
        data_path = Path("raw_data/arXiv-2012.09442v2/anc/data/synthetic/ML_100k_logrank100_offset5.0_scaling1.0")
...
def train_propensity(train_df, vali_df, test_df, flag, num_users, num_items, num_times, popular):
    ...
    # from sklearn.metrics import mean_absolute_error, mean_squared_error # not used
    from sklearn.metrics import root_mean_squared_error
    ...
    for epoch in range(flag.epoch):
        ...
        # mse = mean_squared_error(y_pred=p_pred, y_true=p_true, squared=True) # mse function was removed
        rmse = root_mean_squared_error(p_true, p_pred)
        mse  = rmse ** 2
        ...
        if abs(val_obj) > optim_val_car:
            ...
            model.save_weights(plotpath+ '/' + flag.add + "/saved_model.weights.h5") # keras 3 change
            print("Model saved!")
    model.load_weights(plotpath+ '/' + flag.add + "/saved_model.weights.h5") # keras 3 change
    return model
